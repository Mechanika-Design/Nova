...
class CRC32Stream {
	...
	private static $revlookup = array(
		0,
		128,
		64,
		192,
	...
	public static $default = array("poly"    => 0x04C11DB7,
	                               "start"   => 0xFFFFFFFF,
	                               "xor"     => 0xFFFFFFFF,
	                               "refdata" => 1,
	                               "refcrc"  => 1
	...
	public function Init($options = false) {
		...
		for ($x = 0; $x < 256; $x ++) {
			$c = $this->SHL32($x, 24);
			for ($y = 0; $y < 8; $y ++) {
				$c = $this->SHL32($c, 1) ^ ($c & 0x80000000 ? $poly : 0);
			}
			$this->crctable[$x] = $c;
		}
	...
	public function AddData($data) {
		...
		for ($x < 0; $x < $y; $x ++) {
			if ($this->datareflect) {
				$this->currcrc = $this->SHL32($this->currcrc, 8) ^ $this->crctable[$this->SHR32($this->currcrc, 24) ^ self::$revlookup[ord($data{$x})]];
			} else {
				$this->currcrc = $this->SHL32($this->currcrc, 8) ^ $this->crctable[$this->SHR32($this->currcrc, 24) ^ ord($data{$x})];
			}
		}
