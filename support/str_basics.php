...
// String helper/processing functions.
...
class Str {
		...
		foreach ($data as $key => $val) {
			if (is_string($val)) {
				...
				foreach ($val as $key2 => $val2) {
					$_REQUEST[$key][$key2] = (is_string($val2) ? trim($val2) : $val2);
				}
			} else {
				$_REQUEST[$key] = $val;
			}
	...
	public static function ExtractPathname($dirfile) {
		...
		if ($pos === false) {
			...
			$dirfile = substr($dirfile, 0, $pos + 1);
		}
	...
	public static function LineInput($data, &$pos) {
		...
		if ($pos + 1 < $y && ord($data[$pos]) == $CR && ord($data[$pos + 1]) == $LF) {
			$pos ++;
		}
		if ($pos < $y) {
			$pos ++;
	...
	public static function ConvertBytesToUserStr($num) {
		...
		if ($num < 1048576) {
			return str_replace(".0 ", "", number_format($num / 1024, 1)) . " KB";
		}
		if ($num < 1073741824) {
			return str_replace(".0 ", "", number_format($num / 1048576, 1)) . " MB";
		}
		if ($num < 1099511627776.0) {
			return str_replace(".0 ", "", number_format($num / 1073741824.0, 1)) . " GB";
		}
		if ($num < 1125899906842624.0) {
			return str_replace(".0 ", "", number_format($num / 1099511627776.0, 1)) . " TB";
		}
