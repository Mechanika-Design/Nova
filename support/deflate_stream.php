...
class DeflateStream {
	...
	public static function Compress($data, $compresslevel = - 1, $options = array()) {
		...
		if (!$ds->Init("wb", $compresslevel, $options)) {
			return false;
		}
		if (!$ds->Write($data)) {
			return false;
	...
	public function Init($mode, $compresslevel = - 1, $options = array()) {
		...
		if (!isset($options["type"])) {
			$options["type"] = "rfc1951";
		}

		if ($options["type"] == "rfc1950") {
			$options["type"] = "zlib";
		} else if ($options["type"] == "rfc1952") {
			$options["type"] = "gzip";
		}

		if ($options["type"] != "zlib" && $options["type"] != "gzip" && ($this->compress || $options["type"] != "auto")) {
			$options["type"] = "raw";
		}
		...
		if ($this->compress) {
			$this->filter = stream_filter_append($this->fp, "zlib.deflate", STREAM_FILTER_WRITE, $compresslevel);
		} else {
			$this->filter = stream_filter_append($this->fp, "zlib.inflate", STREAM_FILTER_READ);
		}
		...
		if ($this->compress) {
			if ($this->options["type"] == "zlib") {
				...
				if (!class_exists("CRC32Stream", false)) {
					require_once str_replace("\\", "/", dirname(__FILE__)) . "/crc32_stream.php";
				}
				...
				if (isset($this->options["filename"])) {
					$this->outdata .= str_replace("\x00", " ", $this->options["filename"]) . "\x00";
				}
				if (isset($this->options["comment"])) {
					$this->outdata .= str_replace("\x00", " ", $this->options["comment"]) . "\x00";
				}
			}
		} else {
	...
	public function Finalize() {
		...
		if ($this->compress) {
			if ($this->options["type"] == "zlib") {
				$this->outdata .= pack("N", $this->SHL32($this->options["b"], 16) | $this->options["a"]);
			} else if ($this->options["type"] == "gzip") {
				$this->outdata .= pack("V", $this->options["crc32"]->Finalize()) . pack("V", $this->options["bytes"]);
			}
		}
	...
	private function ProcessOutput() {
		...
		// HACK: Becase ftell() on a stream with a filter is still broken even under the lastest PHP
	...
	private function ProcessInput($final = false) {
		...
		if ($this->options["type"] == "auto") {
			if (strlen($this->indata) >= 3) {
				...
				if (substr($this->indata, 0, 3) === "\x1F\x8B\x08") {
					$this->options["type"] = "gzip";
				} else if ((ord($this->indata{0}) & 0x0F) == 8 && ((ord($this->indata{0}) & 0xF0) >> 4) < 8 && $zlibtest[1] % 31 == 0) {
					$this->options["type"] = "zlib";
				} else {
					$this->options["type"] = "raw";
				}
			} else if ($final) {
				$this->options["type"] = "raw";
			}
		...
		if ($this->options["type"] == "gzip") {
			...
			if (!$this->options["header"]) {
				if (strlen($this->indata) >= 2) {
					...
					if ($cm != 8 || $cinfo > 7 || ($flg & 0x20) || (($cmf << 8 | $flg) % 31) != 0) {
						$this->options["type"] = "ignore";
					} else {
