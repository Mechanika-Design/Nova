...
class WebSocketServer {
	...
	public function Reset() {
		if (!class_exists("WebSocket", false)) {
			require_once str_replace("\\", "/", dirname(__FILE__)) . "/websocket.php";
		}
	...
	public function Start($host, $port) {
		...
		if ($this->fp === false) {
			return array(
				"success"   => false,
				"error"     => self::WSTranslate("Bind() failed.  Reason:  %s (%d)", $errorstr, $errornum),
	...
	public static function FixedStreamSelect(&$readfps, &$writefps, &$exceptfps, $timeout) {
		...
		if (isset($readfps[0]) || isset($writefps[0]) || ($exceptfps !== null && isset($exceptfps[0]))) {
			return false;
		}
		...
		if (isset($writefps[0])) {
			$fps = array();
			foreach ($origwritefps as $key => $fp) {
				$fps[(int) $fp] = $key;
			}
		...
		if ($exceptfps !== null && isset($exceptfps[0])) {
			$fps = array();
			foreach ($origexceptfps as $key => $fp) {
				$fps[(int) $fp] = $key;
			}
	...
	public function Wait($timeout = false, $readfps = array(), $writefps = array(), $exceptfps = null) {
		...
		if ($result2 === false) {
			return array(
				"success"   => false,
				"error"     => self::WSTranslate("Wait() failed due to stream_select() failure.  Most likely cause:  Connection failure."),
					...
					if (strlen($client->readdata) > 100000) {
						...
						while (($pos = strpos($client->readdata, "\n")) !== false) {
							...
							if ($client->request === false) {
								$client->request = trim($line);
							} else if ($line !== "") {
								// Process the header.
								if ($client->lastheader != "" && (substr($line, 0, 1) == " " || substr($line, 0, 1) == "\t")) {
									$client->headers[$client->lastheader] .= $header;
								} else {
									$pos = strpos($line, ":");
									if ($pos === false) {
										$pos = strlen($line);
		...
		foreach ($result["writefps"] as $cid => $fp) {
			...
			if ($client->writedata === "") {
				...
				if ($result2 === false || ($result2 === "" && feof($fp))) {
					...
					$this->ProcessClientQueuesAndTimeoutState($result, $id, true, false, 1);
				} else {
	...
	public function ProcessWebServerClientUpgrade($webserver, $client) {
		if (!($client instanceof WebServer_Client)) {
			return false;
		}

		if (!$client->requestcomplete || $client->mode === "handle_response") {
			return false;
		}
		if ($client->request["method"] !== "GET" || !isset($client->headers["Connection"]) || stripos($client->headers["Connection"], "upgrade") === false || !isset($client->headers["Upgrade"]) || stripos($client->headers["Upgrade"], "websocket") === false) {
			return false;
		}
