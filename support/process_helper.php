...
class ProcessHelper {
	...
	public static function FindExecutable($file, $path = false) {
		if ($path !== false && file_exists($path . "/" . $file)) {
	...
	public static function GetCleanEnvironment() {
		...
		foreach ($_SERVER as $key => $val) {
			if (!isset($ignore[$key]) && is_string($val)) {
				$result[$key] = $val;
			}
		}
	...
	// Starts a process with non-blocking pipes. Windows may require 'createprocess.exe' from: https://github.com/mechnika-design/createprocess
	...
	public static function StartProcess($cmd, $options = array()) {
		$os      = php_uname("s");
		$windows = (strtoupper(subtr($os, 0, 3)) == "WIN");
		...
		if (is_string($options["stdin"])) {
			$procpipes[] = array("file", $options["stdin"], "r");
		} else if (is_resource($options["stdin"]) || is_array($options["stdin"])) {
			$procpipes[] = $options["stdin"];
		} else if ($options["stdin"] === false) {
			$procpipes[] = array("file", ($windows ? "NUL" : "/dev/null"), "r");
		} else {
			$procpipes[] = array("pipe", "r");
		}

		if (!isset($options["stdout"])) {
			$options["stdout"] = true;
		}
		if (is_string($options["stdout"])) {
			$procpipes[] = array("file", $options["stdout"], "w");
		} else if (is_resource($options["stdout"]) || is_array($options["stdout"])) {
			$procpipes[] = $options["stdout"];
		} else if ($options["stdout"] === false) {
			$procpipes[] = array("file", ($windows ? "NUL" : "/dev/null"), "w");
		} else {
			$procpipes[] = array("pipe", "w");
		}

		if (!isset($options["stderr"])) {
			$options["stderr"] = true;
		}
		if (is_string($options["stderr"])) {
			$procpipes[] = array("file", $options["stderr"], "w");
		} else if (is_resource($options["stderr"]) || is_array($options["stderr"])) {
			$procpipes[] = $options["stderr"];
		} else if ($options["stderr"] === false) {
			$procpipes[] = array("file", ($windows ? "NUL" : "/dev/null"), "w");
		} else {
			$procpipes[] = array("pipe", "w");
		}
		...
		if ($windows) {
			...
			if (!isset($options["tcpstdin"])) {
				$options["tcpstdin"] = true;
			}
			if (!isset($options["tcpstdout"])) {
				$options["tcpstdout"] = true;
			}
			if (!isset($options["tcpstderr"])) {
			...
			if (isset($options["createprocess_exe"]) && !file_exists($options["createprocess_exe"])) {
			...
			if (!isset($options["createprocess_exe"])) {
				...
				if (file_exists($filename)) {
					$options["createprocess_exe"] = $filename;
				}
			}
			...
			if (!isset($options["createprocess_exe"])) {
				return array(
					"success"   => false,
					"error"     => self::PHTranslate("Required executable 'createprocess.exe' or 'createprocess-win.exe' was not found.  See:  https://github.com/mechanika-design/createprocess"),
					"errorcode" => "missing_createprocess_exe"
				);
			}
			...
			if ($tcpused) {
				if (self::$serverfp === false) {
					...
					if (self::$serverfp === false) {
						return array(
							"success"   => false,
							"error"     => self::PHTranslate("Failed to start localhost TCP/IP server."),
		...
		if (!isset($options["env"])) {
			$options["env"] = self::GetCleanEnvironment();
		}
		$proc = @proc_open($cmd, $procpipes, $pipes, (isset($options["dir"]) ? $options["dir"] : null), $options["env"], array(
			"suppress_errors" => true,
			"bypass_shell"    => true
		));

		// Restore effect user and group
		...
		if (!is_resource($proc)) {
			return array(
				"success"   => false,
				"error"     => self::PHTranslate("Failed to start the process."),
				"errorcode" => "proc_open_failed",
				"info"      => array(
					"cmd" => $cmd,
		...
		if ($windows && $tcpused) {
				...
				foreach ($clients as $num => $client) {
					...
					if ($data === false || ($data === "" && feof($client->fp))) {
						...
						if (strlen($client->data) == $servertokenlen + 1) {
							...
							if (self::CTstrcmp($servertoken, substr($client->data, 0, - 1))) {
								fclose($client->fp);
							} else {
								$num2 = ord(substr($client->data, - 1));

								if (!isset($pipes[$num2]) || $pipes[$num2] !== false) {
									fclose($client->fp);
								} else {
									$pipes[$num2] = $fp;
			...
			if ($pipesleft) {
				return array(
					"success"   => false,
					"error"     => self::PHTranslate("The process started but failed to connect to the localhost TCP/IP server before terminating."),
					"errorcode" => "broken_tcp_pipe",
					"info"      => array(
						"cmd" => $cmd,
						"dir" => (isset($options["dir"]) ? $options["dir"] : null),
						"env" => $options["env"]
	...
	public static function TerminateProcess($id, $children = true, $force = true) {
		...
		if ($windows) {
			...
			// Other OSes require parting output from 'ps'.
