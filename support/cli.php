...
class CLI {
	public static function ParseCommandLine($options, $args = false) {
		if (!isset($options["shortmap"])) {
			$options["shortmap"] = array();
		}
		if (!isset($options["rules"])) {
			$options["rules"] = array();
		}
		if (!isset($options["userinput"])) {
			$options["userinput"] = false;
		}
		if (!isset($options["allow_opts_after_param"])) {
			$options["allow_opts_after_param"] = true;
		}

		// Clean up shortmap and rules.
		foreach ($options["shortmap"] as $key => $val) {
			if (!isset($options["rules"][$val])) {
				unset($options["shortmap"][$key]);
			}
		}
		foreach ($options["rules"] as $key => $val) {
			if (!isset($val["arg"])) {
				$options["rules"][$key]["arg"] = false;
			}
			if (!isset($val["multiple"])) {
				$options["rules"][$key]["multiple"] = false;
			}
		}
		...
		$chrs = array();
		foreach ($options["shortmap"] as $key => $val) {
			if (isset($options["rules"][$val]) && !$options["rules"][$val]["arg"]) {
				$chrs[$key] = true;
			}
		}
		...
		for ($x = 0; $x < $y; $x ++) {
			...
			if ($allowopt && substr($arg, 0, 1) == "-") {
				...
				if ($pos === false) {
					...
					$optval = substr($arg, $pos + 1);
				}
				$arg2 = substr($arg, 1, $pos - 1);

				if (isset($options["rules"][$arg2])) {
					$opt = $arg2;
				} else if (isset($options["shortmap"][$arg2])) {
					$opt = $options["shortmap"][$arg2];
				} else if ($x == 0) {
					...
					if ($y2 > 0) {
						...
						if ($x2 == $y2) {
							for ($x2 = 0; $x2 < $y2; $x2 ++) {
								...
								if (!$options["rules"][$opt]["multiple"]) {
									$result["opts"][$opt] = true;
								} else {
									if (!isset($result["opts"][$opt])) {
										$result["opts"][$opt] = 0;
									}
									$result["opts"][$opt] ++;
								}
			...
			if ($opt === false) {
				...
				if (!$options["allow_opts_after_param"]) {
					$allowopt = false;
				}
				...
				if (substr($optval, 0, 1) === "\"" || substr($optval, 0, 1) === "'") {
					$optval = substr($optval, 1);
				}
				if (substr($optval, - 1) === "\"" || substr($optval, - 1) === "'") {
					$optval = substr($optval, 0, - 1);
				}
		...
		if ($options["userinput"] !== false && is_string($options["userinput"])) {
			...
			foreach ($result["params"] as $arg) {
				...
				if ($pos === false) {
					...
					if (!isset($result["userinput"][$name])) {
						$result["userinput"][$name] = array();
					}
	...
	public static function GetUserInputWithArgs(&$args, $prefix, $question, $default, $noparamsoutput = "", $suppressoutput = false, $callback = false, $callbackopts = false) {
		...
		do {
			...
			if ($line === false || (is_callable($callback) && !call_user_func_array($callback, array(
						$line,
						&$callbackopts
					)))) {
				if ($line === false) {
	...
	public static function GetLimitedUserInputWithArgs(&$args, $prefix, $question, $default, $allowedoptionsprefix, $allowedoptions, $loop = true, $suppressoutput = false, $multipleuntil = false) {
		...
		do {
			...
			if (is_array($multipleuntil) && $multipleuntil["exit"] === $result) {
				break;
			}
			$result2 = false;
			if (!count($allowedoptions)) {
				break;
			}
			foreach ($allowedoptions as $key => $val) {
				if (!strcasecmp($key, $result) || !strcasecmp($val, $result)) {
					$result2 = $key;
				}
			}
			if ($loop) {
	...
	public static function GetHexDump($data) {
		...
		foreach ($data2 as $line) {
			...
			for ($x2 = 0; $x2 < 16 && $x < $y; $x2 ++) {
				...
				if ($tempchr === 0x09) {
					$result .= "\\t";
				} else if ($tempchr === 0x0D) {
					$result .= "\\r";
				} else if ($tempchr === 0x0A) {
					$result .= "\\n";
				} else if ($tempchr === 0x00) {
					$result .= "\\0";
				} else if ($tempchr < 32 || $tempchr > 126) {
					$result .= "  ";
				} else {
					$result .= " " . $data{$x};
				}
	...
	public static function DisplayError($msg, $result = false, $exit = true) {
		...
		if ($result !== false && is_array($result) && isset($result["error"]) && isset($result["errorcode"])) {
			self::LogMessage("[Error] " . $result["error"] . " (" . $result["errorcode"] . ")", (isset($result["info"]) ? $result["info"] : null));
		}
