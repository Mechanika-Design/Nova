...
class FastCGI {
	...
	public function Connect($host, $port = - 1, $timeout = 10, $async = false) {
		...
		if (!function_exists("stream_socket_client") && !function_exists("fsockopen")) {
			return array("success"   => false,
			             "error"     => self::FCGITranslate("The functions 'stream_socket_client' and 'fsockopen' do not exist."),
		...
		if (!function_exists("stream_socket_client")) {
			if ($this->debug) {
				$this->fp = fsockopen($host, $port, $errornum, $errorstr, $timeout);
			} else {
				$this->fp = @fsockopen($host, $port, $errornum, $errorstr, $timeout);
			}
		} else {
			...
			if ($this->debug) {
				$this->fp = stream_socket_client($host . ($port > - 1 ? ":" . $port : ""), $errornum, $errorstr, $timeout, ($async ? STREAM_CLIENT_ASYNC_CONNECT : STREAM_CLIENT_CONNECT), $context);
			} else {
				$this->fp = @stream_socket_client($host . ($port > - 1 ? ":" . $port : ""), $errornum, $errorstr, $timeout, ($async ? STREAM_CLIENT_ASYNC_CONNECT : STREAM_CLIENT_CONNECT), $context);
			}
		}
		...
		if ($this->fp === false) {
			return array("success"   => false,
			             "error"     => self::FCGITranslate("Unable to establish a connection to '%s'.", $host . ($port > - 1 ? ":" . $port : "")),
	...
	public function RequestUpdatedLimits() {
		...
		if (!$this->client) {
			return array("success"   => false,
			             "error"     => self::FCGITranslate("The UpdateLimits() function is only available in client mode."),
			             "errorcode" => "client_mode_only"
			);
		}
	...
	public function BeginRequest($role, $keepalive = true) {
		...
		if (!$this->client) {
			return array("success"   => false,
			             "error"     => self::FCGITranslate("The BeginRequest() function is only available in client mode."),
			             "errorcode" => "client_mode_only"
			);
		}
		...
		if (count($this->requests) && !$this->multiplex) {
			return array("success"   => false,
			             "error"     => self::FCGITranslate("Multiplexing requests is not allowed."),
			             "errorcode" => "no_multiplexing"
			);
		}
		if (count($this->requests) >= $this->reqlimit) {
			return array("success"   => false,
			             "error"     => self::FCGITranslate("Maximum conncurrent requests reached for this connection."),
			             "errorcode" => "conncurrency_limit"
			);
		}
		...
		if ($x > 65535) {
			return array("success"   => false,
			             "error"     => self::FCGITranslate("No request space left."),
	...
	public function AbortRequest($requestid) {
		...
		if (!$this->client) {
			return array("success"   => false,
			             "error"     => self::FCGITranslate("The AbortRequest() function is only available in client mode."),
			             "errorcode" => "client_mode_only"
			);
		}
		if (!isset($this->requests[$requestid])) {
			return array("success"   => false,
			             "error"     => self::FCGITranslate("The specified request ID does not exist."),
	...
	public function EndRequest($requestid, $appstatus, $protocolstatus = self::PROTOCOL_STATUS_REQUEST_COMPLETE) {
		...
		if ($this->client) {
			return array("success"   => false,
			             "error"     => self::FCGITranslate("The EndRequest() function is only available in server mode."),
	...
	public function SendParams($requestid, $params) {
		...
		if (!$this->client) {
			return array("success"   => false,
			             "error"     => self::FCGITranslate("The SendParams() function is only available in client mode."),
			             "errorcode" => "client_mode_only"
			);
		}
		if (!isset($this->requests[$requestid])) {
	...
	public function SendStdin($requestid, $data) {
		...
		if (!$this->client) {
			return array("success"   => false,
			             "error"     => self::FCGITranslate("The SendStdin() function is only available in client mode."),
			             "errorcode" => "client_mode_only"
			);
		}
		if (!isset($this->requests[$requestid])) {
		...
		if (!$this->requests[$requestid]->stdinopen) {
			return array("success"   => false,
			             "error"     => self::FCGITranslate("The specified request ID has already closed stdin."),
			             "errorcode" => "stdin_closed"
			);
		}
		...
		if ($x < $y || !$y) {
			$this->WriteRecord(self::RECORD_TYPE_STDIN, $requestid, (string) substr($data, $x));
		}
	...
	public function SendStdout($requestid, $data) {
		...
		if ($this->client) {
			return array("success"   => false,
			             "error"     => self::FCGITranslate("The SendStdout() function is only available in server mode."),
			             "errorcode" => "server_mode_only"
			);
		}
		if (!isset($this->requests[$requestid])) {
	...
	public function SendStderr($requestid, $data) {
		...
		if ($this->client) {
			return array("success"   => false,
			             "error"     => self::FCGITranslate("The SendStderr() function is only available in server mode."),
			             "errorcode" => "server_mode_only"
			);
		}
		if (!isset($this->requests[$requestid])) {
	...
	public function SendData($requestid, $data) {
		...
		if ($x < $y || !$y) {
			$this->WriteRecord(self::RECORD_TYPE_DATA, $requestid, (string) substr($data, $x));
		}
	...
	public function Wait($timeout = false) {
		...
		if ($this->emptydisconnect && !count($this->requests) && $this->writedata === "") {
			...
			return array("success"   => false,
			             "error"     => self::FCGITranslate("Connection closed."),
			             "errorcode" => "no_connection"
			);
		}
		...
		if ($result === false) {
			return array("success"   => false,
			             "error"     => self::FCGITranslate("Wait() failed due to stream_select() failure.  Most likely cause:  Connection failure."),
	...
	public function ProcessQueues($read, $write, $readsize = 65536) {
		...
		if ($read) {
			...
			if ($result === false || ($result === "" && feof($this->fp))) {
				return array("success"   => false,
				             "error"     => self::FCGITranslate("ProcessQueues() failed due to fread() failure.  Most likely cause:  Connection failure."),
				             "errorcode" => "fread_failed"
				);
			}
		...
		if ($write) {
			...
			if ($result === false || ($this->writedata === "" && feof($this->fp))) {
				return array("success"   => false,
				             "error"     => self::FCGITranslate("ProcessQueues() failed due to fwrite() failure.  Most likely cause:  Connection failure."),
...
//var_dump($record);
			if ($record["ver"] != 1) {
				return array("success"   => false,
				             "error"     => self::FCGITranslate("Unsupported FastCGI version (%u).", $record["ver"]),
				             "errorcode" => "unsupported_fastcgi_version"
				);
			}
			...
			switch ($record["type"]) {
				case self::RECORD_TYPE_BEGIN_REQUEST:
				{
					if ($this->client) {
						return array("success"   => false,
						             "error"     => self::FCGITranslate("The server attempted to begin a request."),
						             "errorcode" => "server_mode_only"
						);
					}
					if ($record["reqid"] == 0) {
						return array("success"   => false,
						             "error"     => self::FCGITranslate("Client sent an invalid request ID for a new request."),
						             "errorcode" => "invalid_request_id"
						);
					}
					if (strlen($record["content"]) < 3) {
						return array("success"   => false,
						             "error"     => self::FCGITranslate("Client sent invalid record content."),
						             "errorcode" => "invalid_record_content"
						);
					}

					// Ignore requests to start the same ID.
					if ($this->requests[$record["reqid"]]) {
						continue;
					}
				...
				case self::RECORD_TYPE_ABORT_REQUEST:
				{
					if ($this->client) {
						return array("success"   => false,
						             "error"     => self::FCGITranslate("The server attempted to abort a request."),
						             "errorcode" => "server_mode_only"
						);
					}
					if ($record["reqid"] == 0) {
						return array("success"   => false,
						             "error"     => self::FCGITranslate("Client sent an invalid request ID to abort."),
						             "errorcode" => "invalid_request_id"
						);
					}

					if (!isset($this->requests[$record["reqid"]])) {
						continue;
					}
				...
				case self::RECORD_TYPE_END_REQUEST:
				{
					if (!$this->client) {
						return array("success"   => false,
						             "error"     => self::FCGITranslate("The client attempted to end a request."),
						             "errorcode" => "client_mode_only"
						);
					}
					if ($record["reqid"] == 0) {
						return array("success"   => false,
						             "error"     => self::FCGITranslate("Server sent an invalid request ID to end."),
						             "errorcode" => "invalid_request_id"
						);
					}
					if (strlen($record["content"]) < 5) {
						return array("success"   => false,
						             "error"     => self::FCGITranslate("Server sent invalid record content."),
						             "errorcode" => "invalid_record_content"
						);
					}
				...
				case self::RECORD_TYPE_PARAMS:
				{
					if ($this->client) {
						return array("success"   => false,
						             "error"     => self::FCGITranslate("The server attempted to send params."),
						             "errorcode" => "server_mode_only"
						);
					}
					if ($record["reqid"] == 0) {
						return array("success"   => false,
						             "error"     => self::FCGITranslate("Client sent an invalid request ID for adding to 'params'."),
						             "errorcode" => "invalid_request_id"
						);
					}
					...
					if ($record["content"] === "") {
						$request->params = self::ParseNameValues($request->params);
					} else {
						$request->params .= $record["content"];
					}
				...
				case self::RECORD_TYPE_STDIN:
				{
					if ($this->client) {
						return array("success"   => false,
						             "error"     => self::FCGITranslate("The server attempted to send stdin."),
						             "errorcode" => "server_mode_only"
						);
					}
					if ($record["reqid"] == 0) {
						return array("success"   => false,
						             "error"     => self::FCGITranslate("Client sent an invalid request ID for adding to 'stdin'."),
						             "errorcode" => "invalid_request_id"
						);
					}
				...
				case self::RECORD_TYPE_STDOUT:
				{
					if (!$this->client) {
						return array("success"   => false,
						             "error"     => self::FCGITranslate("The client attempted to send stdout."),
						             "errorcode" => "client_mode_only"
						);
					}
					if ($record["reqid"] == 0) {
						return array("success"   => false,
						             "error"     => self::FCGITranslate("Server sent an invalid request ID for adding to 'stdout'."),
						             "errorcode" => "invalid_request_id"
						);
					}
				...
				case self::RECORD_TYPE_STDERR:
				{
					if (!$this->client) {
						return array("success"   => false,
						             "error"     => self::FCGITranslate("The client attempted to send stderr."),
						             "errorcode" => "client_mode_only"
						);
					}
					if ($record["reqid"] == 0) {
						return array("success"   => false,
						             "error"     => self::FCGITranslate("Server sent an invalid request ID for adding to 'stderr'."),
						             "errorcode" => "invalid_request_id"
						);
					}
				...
				case self::RECORD_TYPE_DATA:
				{
					if ($this->client) {
						return array("success"   => false,
						             "error"     => self::FCGITranslate("The server attempted to send filter data."),
						             "errorcode" => "server_mode_only"
						);
					}
					if ($record["reqid"] == 0) {
						return array("success"   => false,
						             "error"     => self::FCGITranslate("Client sent an invalid request ID for adding to 'data'."),
						             "errorcode" => "invalid_request_id"
						);
					}
				...
				case self::RECORD_TYPE_GET_VALUES:
				{
					if ($this->client) {
						return array("success"   => false,
						             "error"     => self::FCGITranslate("The server attempted to get values."),
						             "errorcode" => "server_mode_only"
						);
					}
					...
					foreach ($namevals as $name => $val) {
						if ($name === "FCGI_MAX_CONNS") {
							$namevals2["FCGI_MAX_CONNS"] = (string) $this->connlimit;
						} else if ($name === "FCGI_MAX_REQS") {
							$namevals2["FCGI_MAX_REQS"] = (string) $this->reqlimit;
						} else if ($name === "FCGI_MPXS_CONNS") {
							$namevals2["FCGI_MPXS_CONNS"] = (string) (int) $this->multiplex;
						}
					}
				...
				case self::RECORD_TYPE_GET_VALUES_RESULT:
				{
					if (!$this->client) {
						return array("success"   => false,
						             "error"     => self::FCGITranslate("The client attempted to set values."),
						             "errorcode" => "client_mode_only"
						);
					}
					...
					foreach ($namevals as $name => $val) {
						if ($name === "FCGI_MAX_CONNS") {
							$this->connlimit = (int) $val;
						} else if ($name === "FCGI_MAX_REQS") {
							$this->reqlimit = (int) $val;
						} else if ($name === "FCGI_MPXS_CONNS") {
							$this->multiplex = (bool) (int) $val;
						}
					}
				...
				{
					return array("success"   => false,
					             "error"     => self::FCGITranslate("Unknown record type (%u).", (strlen($record["content"]) ? ord($record["content"][0]) : 0)),
				...
				{
					return array("success"   => false,
					             "error"     => self::FCGITranslate("Unknown or unsupported record type (%u).", ord($record["type"])),
					             "errorcode" => "unknown_record_type"
					);
				}
			}
	...
	protected function ReadRecord() {
		...
		if (strlen($this->readdata) < 8 + $contentlen + $paddinglen) {
			return false;
		}
	...
	protected function WriteRecord($type, $requestid, $content) {
		...
		if ($paddinglen) {
			$data .= str_repeat("\x00", $paddinglen);
		}
