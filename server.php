...
if (isset($args["opts"]["exts"])) {
	$extspath = $args["opts"]["exts"];
} else {
	$extspath = $rootpath . "/extensions";
}

$serverexts = Nova_LoadServerExtensions($extspath);
...
function WriteStartupInfo($result) {
	...
	if (isset($args["opts"]["sfile"])) {
		file_put_contents($args["opts"]["sfile"], json_encode($result, JSON_UNESCAPED_SLASHES));
	}

	if (!$result["success"]) {
		CLI::DisplayError("An error occurred while starting the server.", $result);
	}
}
...
//			$fpmbin = ProcessHelper::FindExecutable("php-fpm7.2", "/usr/sbin");
		...
		while ($result["success"] && !$fcgi->GetRecvRecords()) {
			do {
				...
				if (!$result["success"] || $result["id"] === false) {
					break;
				}
			} while (1);
...
class StatsWebServer extends WebServer {
	protected function HandleResponseCompleted($id, $result) {
		...
		if ($client === false || $client->appdata === false) {
			return;
		}
		...
		if ($client->appdata["cgi"] !== false) {
			foreach ($client->appdata["cgi"]["pipes"] as $fp) {
				fclose($fp);
			}
		...
		if ($client->appdata["file"] !== false && isset($client->appdata["file"]["fp"]) && $client->appdata["file"]["fp"] !== false) {
			fclose($client->appdata["file"]["fp"]);
		}
...
if (!isset($args["opts"]["host"])) {
	$args["opts"]["host"] = "127.0.0.1";
}
if (!isset($args["opts"]["port"])) {
	$args["opts"]["port"] = "0";
}
...
$pos    = strrpos($tempip, ":");
if ($pos !== false) {
	$args["opts"]["port"] = substr($tempip, $pos + 1);
}
...
function ProcessClientCGIRequestBody($request, $body, $id) {
	...
	if ($result === 0 || $difftime > 1.0) {
		...
		if ($sendrate < strlen($client->appdata["cgi"]["stdin"])) {
			$sendrate *= 0.5;
		} else {
			$sendrate *= 1.1;
		}
...
function ProcessClientFastCGIRequestBody($request, $body, $id) {
	...
	if ($diffsize === 0 || $difftime > 1.0) {
		...
		if ($sendrate < $fcgi->GetRawSendQueueSize()) {
			$sendrate *= 0.5;
		} else {
			$sendrate *= 1.1;
		}
...
if (isset($args["opts"]["www"])) {
	$docroot = $args["opts"]["www"];
} else {
...
if ($windows) {
	if (isset($args["opts"]["home"])) {
		...
		if (getenv("ProgramData") !== false) {
			$pfilespath = getenv("ProgramData");
		...
		if (getenv("LOCALAPPDATA") !== false) {
			$ufilespath = getenv("LOCALAPPDATA");
...
if (isset($args["opts"]["app"])) {
	$appname = $args["opts"]["app"];
} else {
...
if (file_exists($pfilespath . "/logs/access.log") && filesize($pfilespath . "/logs/access.log") > 10000000) {
	@unlink($pfilespath . "/logs/access.log");
}
$accessfp = fopen($pfilespath . "/logs/access.log", "ab");

if (file_exists($pfilespath . "/logs/error.log") && filesize($pfilespath . "/logs/error.log") > 100000) {
	@unlink($pfilespath . "/logs/error.log");
}
...
do {
	...
	foreach ($serverexts as $ext) {
		$ext->UpdateStreamsAndTimeout("", $timeout, $readfps, $writefps);
	}
	...
	foreach ($cgis as $id => $val) {
		...
		if ($client->appdata["cgi"]["stdin"] !== "") {
			$writefps["cgi_in_" . $id] = $client->appdata["cgi"]["pipes"][0];
		}

		if (isset($client->appdata["cgi"]["pipes"][1]) && strlen($client->writedata) + strlen($client->appdata["cgi"]["stdout"]) < 262144) {
			$readfps["cgi_out_" . $id] = $client->appdata["cgi"]["pipes"][1];
		}

		if (isset($client->appdata["cgi"]["pipes"][2])) {
			$readfps["cgi_err_" . $id] = $client->appdata["cgi"]["pipes"][2];
		}
	}
	...
	foreach ($fcgis as $id => $val) {
		...
		if ($client->appdata["fcgi"]["conn"]->NeedsWrite()) {
			$writefps["fcgi_send_" . $id] = $client->appdata["fcgi"]["fp"];
		}
	...
	foreach ($result["clients"] as $id => $client) {
		...
		if ($client->appdata["url"] === false) {
			...
			if (substr($client->appdata["url"]["path"], - 1) === "/") {
				$client->appdata["path"] .= "/";
			}
			...
			if ($client->appdata["currext"] === false) {
				...
				$found = false;
				$extra = "";
				if (is_file($docroot . $client->appdata["path"])) {
					$found = $client->appdata["path"];
				} else if (substr($client->appdata["path"], - 4) !== ".php" && is_file($ufilespath . "/www" . $client->appdata["path"])) {
					$found = $client->appdata["path"];
				} else {
					...
					if (is_file($docroot . $path . "index.html")) {
						$found = $path . "index.html";
					} else {
						// Find a parent PHP file.
						while ($path !== "" && !$found) {
							if (is_file($docroot . $path . "index.php")) {
								$found = $path . "index.php";
							} else if ($path !== "/" && is_file($docroot . substr($path, 0, - 1) . ".php")) {
								$found = substr($path, 0, - 1) . ".php";
							} else {
...
//echo $found . "\n";
					...
					if (substr($found, - 4) !== ".php") {
						$client->appdata["file"] = array("name" => (is_file($docroot . $found) ? $docroot : $ufilespath) . $found);
					} else {
						...
						if (isset($client->headers["Content-Length"])) {
...
//exit();
						...
						if ($cgibin !== false) {
							...
							if (isset($args["opts"]["user"])) {
								$options2["user"] = $args["opts"]["user"];
							}
							if (isset($args["opts"]["group"])) {
								$options2["group"] = $args["opts"]["group"];
							}
							...
							// Finalize params.
							if ($result2["success"]) {
								$result2 = $fcgi->SendParams($requestid, array());
							}
			...
			// Remove the receive size limit for Nova.  This doesn't eliminate PHP's file upload limits.
			...
			if ($client->appdata["currext"] !== false) {
				...
				if ($client->mode === "init_response") {
					...
					if ($id2 !== false) {
						...
						if ($client->contenthandled) {
							$data = $client->requestvars;
						} else if (!is_object($client->readdata)) {
							$data = @json_decode($client->readdata, true);
						} else {
				...
				if (!$client->appdata["cgi"]["headersdone"]) {
					...
					while ($pos !== false) {
						...
						if ($pos !== false) {
							...
							if ($name === "status" && (int) $val >= 200) {
								$client->SetResponseCode((int) $val);
							} else if ($name === "content-length" && (int) $val >= 0) {
								$client->SetResponseContentLength((int) $val);
							} else if ($name === "content-type") {
								$client->AddResponseHeader("Content-Type", $val, true);
							} else {
								$client->AddResponseHeader(HTTP::HeaderNameCleanup($name), $val);
							}
						}
...
//var_dump($client->appdata["cgi"]);
				...
				if (!$result2["success"]) {
					...
					if ($client->appdata["fcgi"]["headersdone"]) {
						...
						// When stdout and stderr are both closed, the response is complete.
						if ($request->stdoutcompleted && $request->stderrcompleted) {
							$client->FinalizeResponse();
						}
				...
				if (!isset($client->appdata["file"]["fp"])) {
					...
					if (isset($client->headers["If-Modified-Since"]) && HTTP::GetDateTimestamp($client->headers["If-Modified-Since"]) === $client->appdata["file"]["ts"]) {
						...
						if ($client->appdata["file"]["fp"] === false) {
										...
										foreach ($chunks as $chunk) {
											...
											if (count($chunk) == 2) {
												...
												if ($pos === "" && $pos2 === "") {
													...
													if ($min === false || $min > $size - (int) $pos) {
														$min = $size - (int) $pos;
													}
												} else if ($pos2 === "") {
													if ($min === false || $min > (int) $pos) {
														$min = (int) $pos;
													}
												} else {
													if ($min === false || $min > (int) $pos) {
							...
							if ($start) {
								fseek($client->appdata["file"]["fp"], $start);
							}
							...
							if (isset($mimetypemap[$ext])) {
								$client->SetResponseContentType($mimetypemap[$ext]);
							} else {
								unset($client->responseheaders["Content-Type"]);
							}
...
//				echo "\n";
	...
	if (isset($args["opts"]["quit"]) && $args["opts"]["quit"] >= 60) {
		if ($webserver->NumClients() || $wsserver->NumClients()) {
			$lastclient = microtime(true);
		} else if ($lastclient < microtime(true) - (int) $args["opts"]["quit"]) {
			$running = false;
		}
	}
...
// Terminate PHP-FPM.
if ($cgibin === false && isset($fpminfo)) {
	ProcessHelper::TerminateProcess($fpminfo["pid"]);
}

if (isset($args["opts"]["sfile"])) {
	@unlink($args["opts"]["sfile"]);
}
?>
