...
class CSPRNG {
	...
	public function __construct($cryptosafe = false) {
		...
		if (strtoupper(substr(PHP_OS, 0, 3)) === "WIN") {
			...
			if ($this->mode === false && PHP_VERSION_ID > 50300 && function_exists("mcrypt_create_iv")) {
				$this->mode = "mcrypt";
			}
		} else {
...
//echo $state . " - " . $len . ":  " . $result . "\n";
			switch ($state) {
				...
				case "end":
				{
					if (!isset($freqmap["end"][$len]) || !count($queue) || !isset($freqmap["end"][$len][$queue[count($queue) - 1]])) {
						$state = "restart";
					} else {
				...
				case "recovery":
				{
					if (!count($queue) || !isset($freqmap["recovery"][$queue[count($queue) - 1]])) {
						$state = "restart";
					} else {
