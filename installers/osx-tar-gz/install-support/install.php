...
function DisplayInitErrorDialog($title, $msg, $result = false) {
	...
	if ($mac && ($execfile = ProcessHelper::FindExecutable("osascript", "/usr/bin")) !== false) {
...
if (!$mac) {
	DisplayInitErrorDialog("Invalid OS", "This script only runs on Mac OSX.");
}

// Verify that this is a package.
if (!is_dir($rootpath . "/../app/")) {
	DisplayInitErrorDialog("Missing Application", "This application does not appear to be packaged properly.  Missing 'app' directory.");
}
$basepath = str_replace("\\", "/", rtrim(realpath($rootpath . "/../app/"), "/"));
if (!file_exists($basepath . "/Contents/MacOS/server.php")) {
	DisplayInitErrorDialog("Missing App Server", "This application does not appear to be packaged properly.  Missing 'server.php'.");
}

require_once $basepath . "/Contents/MacOS/support/nova_functions.php";

$appname = false;
$dir     = opendir($basepath . "/Contents/MacOS");
while (($file = readdir($dir)) !== false) {
	if (substr($file, - 7) === ".phpapp") {
		$appname = substr($file, 0, - 7);
	}
}
closedir($dir);
if ($appname === false) {
	DisplayInitErrorDialog("Missing App Startup", "This application does not appear to be packaged properly.  Missing the required .phpapp file.");
}

$packageinfo = json_decode(file_get_contents($basepath . "/Contents/MacOS/package.json"), true);
if (!is_array($packageinfo)) {
	DisplayInitErrorDialog("Missing/Invalid Configuration", "The 'package.json' file is missing or not valid JSON.");
}
...
closedir($dir);
if ($appname === false) {
	CLI::DisplayError("This application does not appear to be packaged properly.  Missing the required .phpapp file.");
}
...
if (!is_dir($basepath)) {
	...
	if (stripos($basepath, "AppTranslocation") !== false) {
