...
class AppleICNS {
	...
	public static function Create($data) {
		...
		if ($info === false) {
			return array("success"   => false,
			             "error"     => self::AICNSTranslate("Unable to load image."),
	...
	public static function ResizeImage(&$data, $destwidth, $destheight) {
		...
		if ($img2 === false) {
			...
			return array("success"   => false,
			             "error"     => self::AICNSTranslate("Unable to resize image."),
			             "errorcode" => "image_crop_resize_failed"
			);
		}
	...
	public static function Parse($data) {
		...
		if ($result["format"] !== "icns") {
			return array("success"   => false,
			             "error"     => self::AICNSTranslate("File is not a valid Apple ICNS file.  Missing 'icns' signature."),
			             "errorcode" => "missing_icns_file_signature"
			);
		}
		...
		while ($x < $y) {
			...
			if (isset(self::$knowntypes[$icon["type"]])) {
				$icon["typeinfo"] = self::$knowntypes[$icon["type"]];
			}
			...
			if (substr($icon["data"], 0, 8) === "\x89PNG\r\n\x1A\n") {
				$icon["format"] = "PNG";
			} else if (substr($icon["data"], 4, 4) === "jP  " || substr($icon["data"], 4, 4) === "jP2 ") {
				$icon["format"] = "JPEG 2000";
			} else if ($icon["type"] === "ic04" || $icon["type"] === "ic05") {
				$icon["format"] = "ARGB";
			} else if ($icon["type"] === "is32" || $icon["type"] === "il32" || $icon["type"] === "ih32" || $icon["type"] === "it32") {
				$icon["format"] = "PackedBits";
			} else {
				$icon["format"] = false;
			}
	...
	public static function Generate($icons) {
		...
		foreach ($icons as $icon) {
			if (!isset($icon["type"]) || strlen($icon["type"]) < 4 || !isset($icon["data"]) || !is_string($icon["data"])) {
				continue;
			}
	...
	protected static function GetBytes(&$data, &$x, $y, $size) {
		...
		if ($x + $size > $y) {
			$result .= str_repeat("\x00", $x + $size - $y);
		}
		$x += $size;
