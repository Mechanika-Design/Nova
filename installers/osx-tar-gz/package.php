...
closedir($dir);
if ($appname === false) {
	echo "Unable to find a .nova file.\n";
...
if (!file_exists($rootpath . "/install_icon.icns")) {
	...
	if (!$result["success"]) {
		echo "Unable to convert '" . $rootpath . "/install_icon.png' to an Apple ICNS file.\n";
	} else {
		file_put_contents($rootpath . "/install_icon.icns", $result["data"]);
	}
}
...
if (!file_exists($basepath . "/" . $appname . ".icns")) {
	...
	$result = AppleICNS::Create($data);
	if (!$result["success"]) {
		echo "Unable to convert '" . $basepath . "/" . $appname . ".png' to an Apple ICNS file.\n";
	} else {
		file_put_contents($basepath . "/" . $appname . ".icns", $result["data"]);
	}
}
...
copy($basepath . "/" . $appname . ".nova", $stagingpath . "/Contents/MacOS/app/Contents/MacOS/" . $appname . ".nova");
...
DirHelper::Copy($basepath . "/support", $stagingpath . "/Contents/MacOS/app/Contents/MacOS/support", true, array(
	$basepath . "/support/createprocess-win.exe" => true,
	$basepath . "/support/messagebox-win.exe"    => true
));
...
$data .= "\t<string>" . htmlspecialchars($appname . ".nova") . "</string>\n";
...
$data .= "\t<string>" . htmlspecialchars($packageinfo["app_name"] . " Installer") . "</string>\n";
$data .= "\t<key>CFBundleIdentifier</key>\n";
$data .= "\t<string>com.nova.installer</string>\n";
$data .= "\t<key>CFBundleVersion</key>\n";
$data .= "\t<string>" . htmlspecialchars($packageinfo["app_ver"]) . "</string>\n";
$data .= "\t<key>CFBundleShortVersionString</key>\n";
$data .= "\t<string>" . htmlspecialchars($packageinfo["app_ver"]) . "</string>\n";
