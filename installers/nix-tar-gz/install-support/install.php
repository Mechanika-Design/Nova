...
if ($windows || $mac) {
	CLI::DisplayError("This script only runs on Linux.");
}

// Verify that this is a package.
$basepath = str_replace("\\", "/", rtrim(realpath($rootpath . "/../"), "/"));
if (!file_exists($basepath . "/server.php")) {
...
if (!function_exists("posix_geteuid")) {
	CLI::DisplayError("Not able to determine what user you are signed in as.  Is this standard Linux?");
}
...
DirHelper::Copy($basepath, $installpath, true, array($basepath . "/install.sh"                  => true,
                                                     $basepath . "/install-support/install.php" => true
));
...
function ScaleAndRegisterImage(&$data, $destwidth, $destheight, $destfile, $destname) {
	...
	if ($info === false) {
		CLI::DisplayError("Unable to load image.");
	}
	$srcwidth  = $info[0];
	$srcheight = $info[1];
...
if ($packageinfo["app_keywords"] != "") {
	$data[] = "Keywords=" . $packageinfo["app_keywords"];
}

$data[] = "Type=Application";
$data[] = "Version=1.1";
$data[] = "Exec=php " . escapeshellarg($installpath . "/" . $appname . ".nova");
