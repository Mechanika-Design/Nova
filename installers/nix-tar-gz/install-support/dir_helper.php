			...
			while (($file = readdir($dir)) != false) {
				if ($file != "." && $file != ".." && !isset($exclude[$path . "/" . $file])) {
					if (is_file($path . "/" . $file) || is_link($path . "/" . $file)) {
						...
						self::Delete($path . "/" . $file, true, $exclude);
					}
				}
			}
			...
			while (($file = readdir($dir)) !== false) {
				if ($file != "." && $file != ".." && !isset($exclude[$srcdir . "/" . $file])) {
					if (is_dir($srcdir . "/" . $file)) {
						if ($recurse) {
							self::Copy($srcdir . "/" . $file, $destdir . "/" . $file, true, true);
						}
					} else {
						...
						if ($fp !== false && $fp2 !== false) {
							while (($data = fread($fp, 1048576)) !== false && $data !== "") {
								fwrite($fp2, $data);
							}
						}
		...
		if ($dir) {
			while (($file = readdir($dir)) !== false) {
				if ($file != "." && $file != ".." && !isset($exclude[$path . "/" . $file])) {
					if (is_dir($path . "/" . $file)) {
						if ($recurse) {
							self::SetPermissions($path . "/" . $file, $dirowner, $dirgroup, $dirperms, $fileowner, $filegroup, $fileperms, true, $exclude);
						}
					} else {
						if ($fileowner !== false) {
							@chown($path . "/" . $file, $fileowner);
						}
						if ($filegroup !== false) {
							@chgrp($path . "/" . $file, $filegroup);
						}
						if ($fileperms !== false) {
							@chmod($path . "/" . $file, $fileperms);
						}
					}
				}
			}
